...
const demoAssets = [
  ...
  { name: 'Brand guidelines 2026', type: 'PDF', size: '12.4 MB', color: 'cream', updated: '3 hrs ago' },
  { name: 'Studio portraits', type: 'RAW', size: '24.7 MB', color: 'violet', updated: 'Yesterday' },
  { name: 'Spring social cutdowns', type: 'MOV', size: '94 MB', color: 'yellow', updated: 'Yesterday' },
]
    ...
    <div className="shell" onClick={() => menu && setMenu(null)}>
      ...
      <main className="main">
        ...
        <section className="content" id="assets">
          ...
          {visibleAssets.length ? <div className={`asset-grid ${view === 'list' ? 'list-view' : ''}`}>{visibleAssets.map((asset) => <article className={`asset-card ${selected.includes(asset.name) ? 'selected' : ''}`} key={`${asset.name}-${asset.updated}`} onClick={() => toggleSelection(asset.name)}><div className={`asset-preview ${asset.color}`}><span className="asset-type">{asset.type}</span><button className="card-menu" onClick={(event) => { event.stopPropagation(); setMenu(menu === asset.name ? null : asset.name) }} aria-label={`More options for ${asset.name}`}>...</button>{menu === asset.name && <div className="menu"><button onClick={() => showNotice(`${asset.name} opened.`)}>Open preview</button><button onClick={() => showNotice(`${asset.name} download queued.`)}>Download</button><button onClick={() => showNotice(`${asset.name} deleted.`)}>Delete</button></div>}<div className="preview-shape" /></div><div className="asset-info"><div><h2>{asset.name}</h2><p>{asset.size} <span>·</span> {asset.updated}</p></div><button className={`star ${starred.includes(asset.name) ? 'starred' : ''}`} onClick={(event) => { event.stopPropagation(); toggleStar(asset.name) }} aria-label={`Star ${asset.name}`}>{starred.includes(asset.name) ? '★' : '☆'}</button></div></article>)}</div> : <div className="empty"><strong>No assets found</strong><span>Try another search or upload a new file.</span></div>}
        </section>
      </main>
